...
# Sync inline Environment=NEXAL_OPS_SECRET from unit file into env file when missing.
if [[ -f "${SERVICE_FILE}" ]]; then
  inline_secret="$(grep -E '^Environment=.*NEXAL_OPS_SECRET=' "${SERVICE_FILE}" | tail -n1 | sed -n 's/.*NEXAL_OPS_SECRET=\([^"'"'"' ]*\).*/\1/p' || true)"
  if [[ -n "${inline_secret}" && "${#inline_secret}" -ge 16 ]]; then
    if ! grep -qE '^NEXAL_OPS_SECRET=' "${ENV_FILE}"; then
      echo "NEXAL_OPS_SECRET=${inline_secret}" >> "${ENV_FILE}"
