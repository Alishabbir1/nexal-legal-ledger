...
def max_users_for_tier(tier: str | None) -> int:
    return PACKAGES[normalize_tier(tier)]["max_users"]


def package_display_label(tier: str | None) -> str:
    info = PACKAGES[normalize_tier(tier)]
    return f"{info['label']} (£{info['monthly_gbp']}/month)"


def user_limit_message(tier: str | None) -> str:
...
def can_add_billable_user(active_count: int, tier: str | None) -> bool:
